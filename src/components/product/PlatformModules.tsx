 ...
 * product word and its one-line tagline from the dictionary.
 *
 * Invoice is left out on purpose: it is the standalone product and has its own
 * row directly below.
 */
export function PlatformModules({ t }: { t: Dict }) {
    ...
    <div className="w-full max-w-[40rem] rounded-[18px] border border-line bg-surface p-5 shadow-[0_30px_70px_-40px_oklch(0.252_0.0592_151.76_/_0.5)] sm:p-6">
      ...
      <ul className="mt-4 grid gap-2 sm:grid-cols-2">
        {modules.map((m) => (
          <li
            key={m.slug}
            className="flex items-center gap-3 rounded-[12px] border border-line bg-bg px-3 py-2.5 sm:last:col-span-2"
          >
